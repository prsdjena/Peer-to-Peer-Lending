...
$sql = mysqli_query($con, "SELECT * FROM borrow WHERE borrowuid='$borrowuid' ") or die(mysqli_error($con));
    ...
    $sql2 = $sql1['collect'] ;
    $needed = $sql1['amount'] - $sql2 ;
if($amount > $needed){
    ?>
    <script>
            alert("Pay lower amount!");
            window.location.href = "P2P.php";
    </script>
<?php
}
else if ($amount <= $balance &&  $amount <= $needed ) {
    ...
    $qry = mysqli_query($con, "INSERT INTO invest (amount, useruid, borrowuid, investuid,date) VALUES ('$amount', '$useruid', '$borrowuid', '$uid','$showDate') ") or die(mysqli_error($con));
